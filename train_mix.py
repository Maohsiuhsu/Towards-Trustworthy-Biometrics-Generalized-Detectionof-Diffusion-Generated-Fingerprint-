...
if __name__ == '__main__':
    ...
    trainer_module = importlib.import_module('networks.multiple_classifier_trainer', __package__)

    if opt.dataset_mode == "normal_v1_2":
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v1_2')    
    elif opt.dataset_mode == 'normal_v3':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v3')
    elif opt.dataset_mode == 'normal_v3_2':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v3_2')
    elif opt.dataset_mode == 'normal_v2_2':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v2_2')    
    elif opt.dataset_mode == 'triple_v2_2':
        TrainerClass = getattr(trainer_module, 'triple_Trainer_v2_2')
    elif opt.dataset_mode == 'normal_v2_3':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v2_3')         
    elif opt.dataset_mode == 'normal_v4_2':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v4_2')
    elif opt.dataset_mode == 'normal_v4_3':
        TrainerClass = getattr(trainer_module, 'mix_Trainer_v4_3')
    elif opt.dataset_mode == 'discriminator':
        TrainerClass = getattr(trainer_module, 'mix_Distrciminator_Trainer')
    else:
    ...
    for epoch in range(opt.niter):
        ...
        for (data1, data2) in itertools.zip_longest(data_loader1, data_loader2, fillvalue=None):
            ...
            model.optimize_parameters(tri_decay=0.1*epoch)
            model.total_steps += 1
        ...
        # acc, ap = validate(model.model, model.classifier, val_opt)[:2]
        acc = 0
